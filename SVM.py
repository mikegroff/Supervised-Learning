...
def learn(array,type='rbf'):

    X = array[:,:-2]
    Y = array[:,-1]
    clf = svm.SVC(C=0.0001, kernel=type, gamma='auto',tol=1e-2)
    clf = clf.fit(X,Y)
    #print("Learning")
    return clf

def crossval(X,Y,size,p=10,k=10,type = 'rbf'):
    score = []
    for i in range(0,k-1):
        ...
        tree = learn(train,type)
        a,b = dt.test(tree,Y)
        c = dt.accr(a,b)
        score.append(c)
    return score

def learningcurve(df,n=100,type = 'rbf'):
    m,z = df.shape
    size = int(0.7*m/n)
    sizes =[]
    traina = []
...
if __name__=="__main__":
    ...
    a,b,c = learningcurve(df1,100,type = 'sigmoid')
    plt.plot(a,b)
    plt.title("SVM(sigmoid) - Collection 1")
    ...
    a,b,c = learningcurve(df2,type = 'sigmoid')
    plt.plot(a,b)
    plt.title("SVM(sigmoid) - Collection 2")
    plt.xlabel("Training Sample Size")
    plt.ylabel("Accuracy")
    plt.legend(["Testing Sample"])
    plt.show()
    ...
    a,b,c = learningcurve(df1)
    plt.plot(a,b)
    plt.title("SVM(rbf) - Collection 1")
    plt.xlabel("Training Sample Size")
    plt.ylabel("Accuracy")
    plt.legend(["Testing Sample"])
    plt.show()
    ...
    plt.plot(a,b)
    plt.title("SVM(rbf) - Collection 2")
    plt.xlabel("Training Sample Size")
    plt.ylabel("Accuracy")
    plt.legend(["Testing Sample"])
    plt.show()
